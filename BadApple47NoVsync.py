...
# .byte 0, 21, 0, 0, 42, 0, 0, 63, 0, 0, 21, 21, 0, 42, 21, 42, 0, 21, 0, 63, 21, 63, 0, 21, 0, 42, 42, 0, 63, 42, 63, 0, 42, 0, 63, 63, 0, 21, 42, 21, 21, 63, 21, 21, 42, 42, 21, 63, 42, 63, 21, 42, 21, 63, 63, 21, 42, 63, 42, 42, 63, 63, 42, 63
    ...
    #output_file = "SteamboatWillieExtraBW.bin"
    RLEcount=0
    prev_byte=64
    mybyte=b'1'
    ...
    with open(output_file, 'ba') as g:
        ...
        while FileCounter < 6570: #The python script at 30fps outputs 6,571 frames. 13257: for steamboat
            ...
            #OK,lets try outputting a vsync packet every frame.
            #I'll prob setup a way to skip vsync near 'problem' frames.
            #print( ("BadApple" + str(FileCounter) + ".txt"),',', VChangeCounter)
            # print('CangeCounter: ', VChangeCounter)
            # if VChangeCounter<VChangeLimit:
            #     if VChangeCounter < VChangeLimit2:
            #         g.write(bytes([TVsync2]))  # V-sync 'beep'/RUN LENGTH
            #         g.write(bytes([TBeep]))  # 'Beep' token/color
            #         VchangeTwo+=1
            #     else:
            #         g.write(bytes([TVsync]))  # V-sync 'beep'/RUN LENGTH
            #         g.write(bytes([TBeep]))  # 'Beep' token/color
            #         VchangeOne+=1
            # else:
            #     VchangeSkip+=1
            #     # print('FileName    : ', ("BadApple" + str(FileCounter) + ".txt"))
            #     # print('CangeCounter: ', VChangeCounter)
            #     # print('Skip Vsync  : ', VchangeSkip)
            #     # print('One Vsync   : ', VchangeOne)
            #     # print('Two Vsync   : ', VchangeTwo)
            # VChangeCounter = 0

            #g.write(bytes(b'00')) #V-sync 'beep'/RUN LENGTH
            #g.write(bytes(b'ff')) #'Beep' token/color
