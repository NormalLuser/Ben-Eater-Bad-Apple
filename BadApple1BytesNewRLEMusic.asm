...
; Add RLA Count to screen pointer
  clc 
  tya 
...
.Vsync: ; I use assembler ' if ' statments here for 
        ; Vsync and a debug 'slow' pixel
  if VsyncOn=1
    ldx VGAClock ;IRQ does a DEC on Vsync
    cpx #250     ;Using this as 'zero' sync
    bcs .EGVsync ; = > 250 wait or sync
...
.Beep:
    ...
    ;some number of Audio packets before starting to send frames.
      ...
      ;Last thing
      INC BeepCount
      ;BEQ .StartBeep ;Full Buffer lets start
...
.BeepExit
  ...
  .org $700 ;RLERender must start on page!
.RLERender: ;This is around 8.2 cycles a pixel vrs 13.3 for a loop
 ...
 ; I could Bit shift and ADC then subtract instead of using this table,
 ; but that would take more cycles than this.
  .ORG $800 ;Page align
.RLEArray: ;Decending Array of 3 byte skips 
; I don't think I actually use zero? 
; I should for ' 1 ' but whatever just dup first entry for now.
...
;********************* Jump to video stream decode now! *********************
  jmp .readloopStart ;Decoder entry point. Toss some frames on that screen!
...
; delay.. :) so I'll just comment out for now?
;.delay:
;   ldx #0
;   ldy #0
; .dloop
