...
# This started as a two byte based RLE encoder of arbitrary data.
# This version is a 1 byte encoder that can only do 4 color encodes.
# 30 Frames a Second original video extract.
# 53.4 Frames a Second on Average without vsync on decode
...
#
# Support for encoding Beeps in the stream dropped.
# any music will be with a ROM based routine.
#
...
#      the table at the bottom of main() is the lookup for this.

def main():
    # RLE length - 3 for Tri Pixel. TriPixel coding could be modified to increase Max Bytes to 64.
    MAX_BYTE_COUNT = 61
    MAX_SKIP_COUNT = 127
    LastColor=253
    LastTriMatch=False

    output_file = "BadAppleNew1Byte8-3.bin"
    #output_file = "SteamboatWillieExtraBW1byte8.bin"
    RLEcount=0
    prev_byte=128
    mybyte=b'1'
    ...
    # start at frame 29 because first 30 frames are blank.
    FileCounter=29
    ...
    #This version of the encoder will just assume valid B/W/DG/LG colors.
    ...
    with open(output_file, 'ba') as g:
        ...
        while FileRepeate<1: #Change this to make a bigger 'looped' file
            ...
            while FileCounter < 6570: #13257: #The python script at 30fps outputs 6,571 frames. 13257: for steamboat
                ...
                #print(FileCounter,",",FrameByteCounter)
                ...
                while Previousbyte1!=b'':
                    if StartTri==True:
                        ...
                        if TriCount==3:
                            ...
                            #Lets hack in a test to see if all of the tripixel match the
